...
def retake_work(workload_list, slot):
	...
	open(slotqueue_filename(slot),'w').close()	#write empty file

def submit(slot, timelimit=JOB_TIMELIMIT):
	if is_slot_queue_empty(slot):
		return

	if slot in slot2jobid:
		del jobid2slot[slot2jobid[slot]]	#slot2jobid[slot] is overridden below.
	walltime=timelimit + BUFFER
	jobdesc="msub -q singlenode -l nodes=1:ppn=1,pmem=62gb,walltime=" + str(walltime) + "naccesspolicy=singlejob " + my_dir + "/bwclusterwrapper.sh \"" + my_dir + "/smallworkqueue_worker.sh " + slotqueue_filename(slot) + "\""
	#print(jobdesc)
...
def manage_jobs(try_showq):
	...
	slots_with_empty_queue.extend(available_slots)
	...
	#chunk_sizes_of_available_slots = [len(x) for x in chunked_tasks[num_active_slots_with_empty_queue:]]
	del remaining_work[:num_tasks_to_distribute]
	if num_tasks_to_distribute > 0:	#save some IO, huh?
...
def should_i_terminate():
	return len(remaining_work) == 0 and all([is_slot_queue_empty(s) for s in range(MAX_JOBS_IN_QUEUE)])
...
for s in range(MAX_JOBS_IN_QUEUE):
	Path(slotqueue_filename(s)).touch()
...
while True:
	...
	if should_i_terminate():
		#for s in range(MAX_JOBS_IN_QUEUE):
		#	os.remove(slotqueue_filename(s))	#clean up after yourself, mate
		break
	else:
